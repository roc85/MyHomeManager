package com.zp.myhomemanager.recog;
...
import com.zp.myhomemanager.recog.listener.RecogEventAdapter;
...
public class MyRecognizer {
    ...
    public MyRecognizer(Context context, EventListener eventListener) {
        if (isInited) {
            MyLog.e(TAG, "还未调用release()，请勿新建一个新类");
    ...
    public void loadOfflineEngine(Map<String, Object> params) {
        ...
        MyLog.i(TAG + ".Debug", "离线命令词初始化参数（反馈请带上此行日志）:" + json);
    ...
    public void start(Map<String, Object> params) {
        // SDK集成步骤 2.1 拼接识别参数
        String json = new JSONObject(params).toString();
        MyLog.i(TAG + ".Debug", "识别参数（反馈请带上此行日志）" + json);
