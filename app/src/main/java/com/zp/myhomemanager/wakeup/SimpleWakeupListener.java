package com.zp.myhomemanager.wakeup;
...
public class SimpleWakeupListener implements IWakeupListener {
    ...
    public void onError(int errorCode, String errorMessge, WakeUpResult result) {
        MyLog.i(TAG, "唤醒错误：" + errorCode + ";错误消息：" + errorMessge + "; 原始返回" + result.getOrigalJson());
    }

    @Override
    public void onASrAudio(byte[] data, int offset, int length) {
        MyLog.e(TAG, "audio data： " + data.length);
    }
